...
/* ── destino de los pedidos ──────────────────────────────────────
   ...
   (ej. "51999888777"). Déjalo vacío hasta tenerlo — ese botón queda
   inactivo mientras tanto; el correo funciona igual, en automático. */
...
function pintarFinal(){
  ...
  $('#finalFormula').innerHTML = f.filas.map((r, k) => `
    <li class="f-row" style="--c:${r.ing.color}">
      <span class="f-name">${r.ing.nombre}</span>
      <span class="f-pct">${r.pct}%</span>
      <span class="f-bar"><i style="--bd:${(k * .09 + .25).toFixed(2)}s"></i></span>
    </li>`).join('');
  requestAnimationFrame(() => {
    $$('#finalFormula .f-bar i').forEach((el, k) => el.style.width = f.filas[k].pct + '%');
  });
  ...
  const envase = E.genero === 'mujer' ? 'dama.png' : 'varon.png';
  $('#extrasBlock').hidden = extras.length === 0;
  $('#extraVials').innerHTML = extras.map(i => `
    <article class="extra-vial">
      <img src="img/${envase}" alt="Envase MANTARO para ${i.nombre}">
...
const camposEtiqueta = [
  ['fragName','fragCount','labelName','TU NOMBRE'],
  ['fragCreator','creatorCount','labelCreator','Tu nombre'],
  ['fragDedication','dedicationCount','labelDedication','Tu dedicatoria']
];
camposEtiqueta.forEach(([campo, contador, destino, respaldo]) => {
  $(`#${campo}`).addEventListener('input', e => {
    $(`#${contador}`).textContent = e.target.value.length;
    $(`#${destino}`).textContent = e.target.value.trim() || respaldo;
...
function pintarBolsa(){
  ...
  cuerpo.innerHTML = E.bolsa.length
    ? E.bolsa.map(i => `
      <div class="bag-item">
        <span class="bag-thumb" style="--c1:${Flacon.aclarar(i.color,.3)};--c2:${Flacon.aclarar(i.color,-.42)}"></span>
        <span>
          <span class="bi-name">${i.nombre}</span>
          <span class="bi-meta">${i.ml} ml</span>
          <span class="bi-notes">${i.notas}</span>
        </span>
...
$('#addCart').addEventListener('click', () => {
  ...
  E.bolsa.push({
    ...
    ml: E.volumen.ml,
    color: f.color,
    notas: `Mezcla: ${f.filas.map(r => `${r.ing.nombre} ${r.pct}%`).join(' · ')}`
...
$('#restart').addEventListener('click', () => {
  ...
  camposEtiqueta.forEach(([campo, contador]) => { $(`#${campo}`).value = ''; $(`#${contador}`).textContent = '0'; });
  $('#labelName').textContent = 'TU NOMBRE';
  $('#labelCreator').textContent = 'Tu nombre';
  $('#labelDedication').textContent = 'Tu dedicatoria';
  refrescarFrasco();
  ir('genero');
});
